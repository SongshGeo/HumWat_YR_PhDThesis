...
水的“稀缺情况”不仅取决于气候能提供稀缺情况资源（而且在许多地区日益稀缺和不确定性），还取决于灌溉和工业等经济活动的需求（日益无法被满足），更可以被水库蓄水、跨流域调水等工程所人为改变\cite{qin2019,wada2014,huang2021}。
...
\subsection{相关分析与统计检验}

在统计学中，皮尔逊相关系数（Pearson correlation coefficient，通常用 r 表示）是用于衡量两个连续变量之间线性关系强度的指标。它的值范围在 -1 到 1 之间，其中 -1 表示完全负相关，1 表示完全正相关，0 表示无相关性。为了计算两个变量之间的皮尔逊相关系数，我们可以使用以下公式：
...
本研究中使用如下方法计算某时段某子指标（$X$）对总指标变化（或指标/地区均值）的贡献度$Contribution_{X}$：
...
本研究中许多综合指标都是子指标的等权平均，例如表征稀缺情况的SFV指数就是稀缺性（S）、韧性（F）、易变性（V）三个指标的平均，综合水治理指标（IWGI）则是稀缺情况、使用目的、分配方式三个子指标的平均，因此上述计算方法可以根据实际分析需求调整，以计算不同时间段、不同指标的贡献度，帮助了解各指标在特定时间段内对总体变化的影响程度。
